...
const VerifierDashboard = () => {
  const [stats, setStats] = useState({ pending: 0, approvedToday: 0, flagged: 0 });
  ...
  useEffect(() => {
    const loadDashboard = async () => {
      try {
        ...
        const pending = logsData.filter((l: any) => (l.status || '').toLowerCase() === 'pending');
        const approved = logsData.filter((l: any) => (l.status || '').toLowerCase() === 'verified');
        const rejected = logsData.filter((l: any) => (l.status || '').toLowerCase() === 'rejected');
  ...
  return (
    <div className="bg-background font-body text-on-surface overflow-x-hidden min-h-screen">
        ...
        <div className="grid grid-cols-12 gap-6 mb-10">
          ...
          <div className="col-span-12 md:col-span-6 lg:col-span-3 bg-surface-container-lowest p-8 rounded-3xl flex flex-col justify-between transition-all hover:scale-[1.02] border border-transparent hover:border-emerald-500/30 group shadow-sm">
            <div>
              ...
              <p className="text-on-surface-variant font-medium text-sm">Approved Today</p>
              <h4 className="text-4xl font-extrabold font-headline text-on-surface mt-1">48</h4>
            </div>
            <div className="mt-4 text-emerald-600 font-bold text-xs flex items-center gap-1">
              <span className="material-symbols-outlined text-sm">arrow_upward</span>
              Verified 92% of target
            </div>
          </div>
          ...
          <div className="col-span-12 md:col-span-6 lg:col-span-3 bg-error-container p-8 rounded-3xl flex flex-col justify-between transition-all hover:scale-[1.02] border border-transparent hover:border-error/30 group shadow-sm">
            <div>
              ...
              <p className="text-on-error-container font-medium text-sm">Flagged Anomalies</p>
              <h4 className="text-4xl font-extrabold font-headline text-on-error-container mt-1">07</h4>
            </div>
            ...
            <div className="bg-surface-container-lowest rounded-3xl p-8 flex-1 shadow-sm border border-outline-variant/10">
              ...
              <div className="space-y-6 relative">
                ...
                <div className="relative pl-10">
                  <div className="absolute left-0 top-1 w-5 h-5 rounded-full bg-emerald-600 flex items-center justify-center border-4 border-surface-container-lowest">
                    <span className="material-symbols-outlined text-[10px] text-white">done</span>
                  </div>
                  <p className="text-sm font-bold text-on-surface">Resolved Dispute #FG-8812</p>
                  <p className="text-xs text-on-surface-variant mt-1">Status: <span className="text-emerald-600 font-semibold">Verified</span> • 12 mins ago</p>
                </div>

                <div className="relative pl-10">
                  <div className="absolute left-0 top-1 w-5 h-5 rounded-full bg-error flex items-center justify-center border-4 border-surface-container-lowest">
                    <span className="material-symbols-outlined text-[10px] text-white">flag</span>
                  </div>
                  <p className="text-sm font-bold text-on-surface">Flagged Anomalous Payout</p>
                  <p className="text-xs text-on-surface-variant mt-1">Platform: Uber • Escalated to L2 • 45 mins ago</p>
                </div>

                <div className="relative pl-10">
                  <div className="absolute left-0 top-1 w-5 h-5 rounded-full bg-secondary text-white flex items-center justify-center border-4 border-surface-container-lowest">
                    <span className="material-symbols-outlined text-[10px]">person</span>
                  </div>
                  <p className="text-sm font-bold text-on-surface">New Account Verification Started</p>
                  <p className="text-xs text-on-surface-variant mt-1">Worker: Daniel K. • In Progress • 1 hour ago</p>
                </div>
              </div>
